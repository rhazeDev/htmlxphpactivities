...
<head>
    ...
    <title>Add Customer - Car Rentals</title>
...
<body>
    <div class="form-container">
        ...
        if ($returnPage === 'rentals') {
            echo '<a href="rentals.php" class="back-link">';
            echo '<i class="fas fa-arrow-left"></i>';
            echo 'Back to Add Rental';
            echo '</a>';
        } else {
            echo '<a href="../dashboard.php?table=Customers" class="back-link">';
            echo '<i class="fas fa-arrow-left"></i>';
            echo 'Back to Customers List';
            echo '</a>';
        }
        ?>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        while ($fieldinfo = $fieldResult->fetch_field()) {
            ...
            if (strpos($fieldName, 'Date') !== false) {
                $types .= 's';
            } elseif (strpos($fieldName, 'ID') !== false || $fieldName == 'Age') {
                $types .= 'i';
            } else {
                $types .= 's';
            }
    ...
    while ($fieldinfo = $fieldResult->fetch_field()) {
        ...
        if ($label == 'LicenseImg') {
            echo '<label for="license-upload" class="image-upload" style="cursor: pointer; display: block;">';
            echo '<i class="fas fa-upload" style="font-size: 2rem; color: var(--text-secondary); margin-bottom: 1rem;"></i>';
            echo '<p id="license-upload-text" style="margin-bottom: 1rem; color: var(--text-secondary);">Click to upload license image</p>';
            echo '<input type="file" id="license-upload" name="' . $label . '" accept="image/*" style="display: none;" required>';
            echo '</label>';
            echo '<div id="license-file-name" style="margin-top: 0.5rem; color: var(--success-color); font-size: 0.875rem; display: none;"></div>';
            echo '</div>';
            continue;
        }
        ...
        if (stripos($label, 'contact') !== false || stripos($label, 'phone') !== false || stripos($label, 'mobile') !== false) {
            echo '<input type="text" name="' . $label . '" placeholder="09XX-XXX-XXXX" maxlength="13" oninput="formatContactNumber(this)" onblur="validateContactNumber(this)" required>';
            echo '</div>';
            continue;
        } elseif (stripos($label, 'license') !== false && stripos($label, 'number') !== false) {
            echo '<input type="text" name="' . $label . '" placeholder="XXX-XX-XXXXXX" maxlength="13" oninput="validateLicenseNumber(this)" onblur="validateLicenseNumber(this)" required>';
            echo '</div>';
            continue;
        } elseif (stripos($label, 'email') !== false) {
            echo '<input type="email" name="' . $label . '" placeholder="example@email.com" oninput="validateEmail(this)" onblur="validateEmail(this)" required>';
            echo '</div>';
            continue;
        } elseif (stripos($label, 'name') !== false) {
            echo '<input type="text" name="' . $label . '" placeholder="Full Name" oninput="validateName(this)" onblur="validateName(this)" required>';
            echo '</div>';
            continue;
        } elseif (stripos($label, 'age') !== false) {
            echo '<input type="number" name="' . $label . '" min="18" max="100" placeholder="18-100" oninput="validateAge(this)" onblur="validateAge(this)" required>';
            echo '</div>';
            continue;
        } elseif (stripos($label, 'address') !== false) {
            $placeholder = 'placeholder="Complete Address"';
            $extraAttributes = 'minlength="10"';
        }
    ...
    echo '<input type="submit" value="Add Customer" class="btn btn-primary">';
    echo '</div>';
    echo '</form>';
    ?>
        </div>
    ...
    <script>
        document.getElementById('license-upload').addEventListener('change', function(e) {
            ...
            const uploadText = document.getElementById('license-upload-text');
            const fileNameDiv = document.getElementById('license-file-name');
            
            if (fileName) {
                uploadText.textContent = 'License image selected:';
                fileNameDiv.textContent = fileName;
                fileNameDiv.style.display = 'block';
            } else {
                uploadText.textContent = 'Click to upload license image';
