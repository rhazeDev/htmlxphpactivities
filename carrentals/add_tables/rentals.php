        ...
        function updateAmountToPay(totalCost) {
            ...
            if (paymentMethod && amountToPayField) {
                if (paymentMethod.value === 'Downpayment') {
                    const minDownpayment = totalCost * 0.50; amountToPayField.min = minDownpayment.toFixed(2);
...
<body>
                ...
                if (isset($validationError) && $validationError) {
                    ...
                    if (!empty($fields)) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            if ($kmBefore > 0) {
                                $distanceQuery = $conn->prepare("INSERT INTO Distances (RentalID, VehicleID, KmBefore, KmAfter, KmUsed, DateRecorded) VALUES (?, ?, ?, 0, 0, ?)");
                                $currentDate = date('Y-m-d');
                                $distanceQuery->bind_param('iiis', $newRentalId, $vehicleID, $kmBefore, $currentDate);
    ...
    <script>
        document.getElementById('car-image-upload').addEventListener('change', function (e) {
